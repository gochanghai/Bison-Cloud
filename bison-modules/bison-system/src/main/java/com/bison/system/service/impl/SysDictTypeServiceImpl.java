...
import com.bison.common.core.exception.CustomException;
...
import com.bison.system.mapper.SysDictTypeMapper;
...
public class SysDictTypeServiceImpl extends ServiceImpl<SysDictTypeMapper, SysDictType> implements ISysDictTypeService {
    ...
    @PostConstruct
    public void init() {
        List<SysDictType> dictTypeList = this.baseMapper.selectDictTypeAll();
    ...
    public int deleteDictTypeByIds(Long[] dictIds) {
        ...
        int count = this.baseMapper.deleteDictTypeByIds(dictIds);
        if (count > 0) {
    ...
    public int insertDictType(SysDictType dictType) {
        int row = this.baseMapper.insertDictType(dictType);
        if (row > 0) {
            DictUtils.clearDictCache();
        }
        return row;
    ...
    public int updateDictType(SysDictType dictType) {
        SysDictType oldDict = this.baseMapper.selectDictTypeById(dictType.getDictId());
        dictDataMapper.updateDictDataType(oldDict.getDictType(), dictType.getDictType());
        int row = this.baseMapper.updateDictType(dictType);
        if (row > 0) {
            DictUtils.clearDictCache();
        }
        return row;
